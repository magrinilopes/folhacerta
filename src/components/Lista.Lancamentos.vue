  ...
  <div>
    ...
    <div style="overflow-x: auto">
      <table class="table table-stripped table-hover table-todos-lancamentos">
        ...
        <tbody>
          <tr v-for="lanc in lancamentos">
            <td>{{ lanc.NomeFuncionario }}</td>
            <td>{{ lanc.CodigoFuncional }}</td>
            <td>{{ lanc.CodigoJuncao }}</td>
            <td>{{ lanc.DescricaoJuncao }}</td>
            <td>{{ lanc.Gestor }}</td>
            <td>{{ lanc.EmailGestor }}</td>
            <td>{{ lanc.Tipo }}</td>
            <td>{{ showOnlyDate(lanc.DataLancamento) }}</td>
            <td>{{ showOnlyDate(lanc.DataInicio) }}</td>
            <td>{{ showOnlyDate(lanc.DataFim) }}</td>
            <td>{{ getBoolean(lanc.AfastamentoInss) }}</td>
            <td>{{ lanc.Cid }}</td>
            <td>{{ lanc.TotalDiasAfastado }}</td>
            <td>{{ showOnlyDate(lanc.DecimoSextoDia) }}</td>
            <td>{{ lanc.Status }}</td>
            <td>
              <div class="btn btn-primary" style="font-size: 0.7em; font-weight: bold; padding: 3px 12px;" @click="selecionarLancamento(lanc)">VER</div>
            </td>
          </tr>
        </tbody>
      </table>
...
<script>
  ...
  export default {
    ...
    computed: Object.assign({},
      mapGetters(['getToken', 'getOrderListaLancamento']),
      {
        sort(){
          return this.getOrderListaLancamento
        }
      }
    ),
    methods: Object.assign({},
      mapActions(['getLancamentos','getTipoAfastamento', 'getStatus']),
      {
        changeOrder(field){
          if(this.sort[0] == field){
            let dir = this.sort[1] == 'asc' ? 'desc' : 'asc';
            this.$store.commit('SET_ORDER_LISTALANCAMENTO', [field, dir]);
          }
          else{
            let dir = this.sort[1];
            this.$store.commit('SET_ORDER_LISTALANCAMENTO', [field, dir]);
        ...
        loadData(){
          ...
          this['getLancamentos'](obj).then(ret => {
            if(ret.ok){
              this.lancamentos = ret.data.Data;
            }
          });
        },
        selecionarLancamento(lanc){
          this.$router.push({ name: 'timeline', params: { userToken: lanc.TokenApp }});
        }
      }
    )
  }
